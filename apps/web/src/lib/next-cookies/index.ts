...
const isContextFromAppRouter = (
  ...
  return (
    (!!context?.req &&
      ...
      isCookiesFromAppRouter(context?.req.cookies)) ||
    (!!context?.res &&
      "cookies" in context.res &&
      isCookiesFromAppRouter(context?.res.cookies)) ||
    (!!context?.cookies && isCookiesFromAppRouter(context.cookies()))
  );
};

const transformAppRouterCookies = (
  cookies: AppRouterCookies
): TmpCookiesObj => {
...
export const getCookies = (options?: OptionsType): TmpCookiesObj => {
  ...
  for (let i = 0, len = documentCookies.length; i < len; i++) {
    ...
    if (!cookieParts) continue;
    const _cookie = cookieParts.slice(1).join("=");
    const name = cookieParts[0] as keyof TmpCookiesObj;
